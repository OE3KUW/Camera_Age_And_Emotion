...
byte ReceiveState=0,cmdState=1,strState=1,questionstate=0,
...
void setup() 
{
  ...
  // sept   initSPIFFS();
    ...
    config.pin_sccb_sda = SIOD_GPIO_NUM; //pin_sscb_sda = SIOD_GPIO_NUM;
    config.pin_sccb_scl = SIOC_GPIO_NUM; //pin_sscb_scl = SIOC_GPIO_NUM;
...
void getCommand(char c)
{
    ...
    if (ReceiveState == 1) 
    {
        ...
        if (c == ';') strState++;
        if ((cmdState == 1) && ((c != '?') || 
            (questionstate == 1))             ) cmd = cmd + String(c);
        if ((cmdState == 0) && (strState == 1) && 
            ((c != '=') || (equalstate == 1))     )           P1 = P1 + String(c);
        ...
        if ((cmdState == 0) && (strState >= 9) && 
            ((c != ';') || (semicolonstate==1))   )           P9 = P9 + String(c);   
